...
function getTransport() {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.warn(
      "[email] GMAIL_USER / GMAIL_APP_PASSWORD not set — emails disabled",
    );
    return null;
  }
...
async function send({ to, subject, html }) {
  try {
    ...
    if (!t) return;
    await t.sendMail({
      from: `WorkNest <${process.env.GMAIL_USER}>`,
      to,
      subject,
      html,
    });
...
function template(heading, body, ctaText, ctaUrl) {
  return `<div style="font-family:sans-serif;max-width:520px;margin:auto;padding:24px">
    ...
    <p style="color:#9ca3af;font-size:11px;margin-top:32px">WorkNest — Milestone Escrow Platform</p>
  </div>`;
}

export async function emailMilestoneFunded({
  freelancerEmail,
  projectTitle,
  milestoneTitle,
  amount,
  ...
  await send({
    to: freelancerEmail,
    subject: `[WorkNest] Milestone funded — "${milestoneTitle}"`,
    html: template(
...
export async function emailMilestoneSubmitted({
  clientEmail,
  projectTitle,
  milestoneTitle,
  amount,
  ...
  await send({
    to: clientEmail,
    subject: `[WorkNest] Work submitted — "${milestoneTitle}"`,
    html: template(
...
export async function emailMilestoneApproved({
  freelancerEmail,
  projectTitle,
  milestoneTitle,
  amount,
  ...
  await send({
    to: freelancerEmail,
    subject: `[WorkNest] Payment released — "${milestoneTitle}"`,
    html: template(
      `₹${amount.toLocaleString()} released to your wallet`,
      `The client approved your submission for "${milestoneTitle}" in <b>${projectTitle}</b>. The payment has been credited to your WorkNest wallet.`,
      "View Project",
      projectLink(projectId),
    ),
  });
...
export async function emailMilestoneDisputed({
  clientEmail,
  freelancerEmail,
  adminEmail,
  projectTitle,
  ...
  for (const to of [clientEmail, freelancerEmail, adminEmail].filter(Boolean)) {
    await send({
      to,
      subject: `[WorkNest] Dispute raised — "${milestoneTitle}"`,
      html: template(
...
export async function emailMilestoneRefunded({
  clientEmail,
  projectTitle,
  milestoneTitle,
  amount,
  ...
  await send({
    to: clientEmail,
    subject: `[WorkNest] Refund processed — "${milestoneTitle}"`,
    html: template(
