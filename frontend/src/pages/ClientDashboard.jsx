...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    projects
      ...
      .forEach((p) => {
        api
          .get(`/api/reviews/${p._id}/mine`)
          .then(({ data }) => {
            if (data.review)
              setReviews((r) => ({
                ...r,
                [p._id]: { ...r[p._id], submitted: true },
              }));
          })
          .catch(() => {});
      });
  ...
  function addMilestone() {
    setMilestones((m) => [...m, { ...EMPTY_MILESTONE }]);
  }
  ...
  function updateMilestone(idx, field, value) {
    setMilestones((m) =>
      m.map((item, i) => (i === idx ? { ...item, [field]: value } : item)),
    );
  }
  ...
  async function handleReview(projectId) {
    ...
    setReviews((r) => ({
      ...r,
      [projectId]: { ...r[projectId], submitting: true },
    }));
    try {
      await api.post(`/api/reviews/${projectId}`, {
        rating: rev.rating,
        comment: rev.comment ?? "",
      });
      ...
      setReviews((r) => ({
        ...r,
        [projectId]: { ...r[projectId], submitting: false, submitted: true },
      }));
    } catch (err) {
      toast.error(err.response?.data?.error ?? "Review failed");
      setReviews((r) => ({
        ...r,
        [projectId]: { ...r[projectId], submitting: false },
      }));
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-4 sm:px-6 py-4">
        <h1 className="text-xl font-bold text-indigo-700">WorkNest</h1>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-gray-300 bg-white p-10 text-center">
              <p className="text-gray-400 text-sm">
                No projects yet — post your first one above.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                >
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${PROJECT_BADGE[p.state] ?? ""}`}
                      >
                        {p.state}
                      </span>
                      {p.state !== "POSTED" && (
                        ...
                        >
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-2 flex-wrap min-w-0">
                              <span
                                className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${M_BADGE[m.state] ?? ""}`}
                              >
                                {m.state}
                              </span>
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              {m.state === "CREATED" && (
                                <>
                                  ...
                                  <span
                                    ...
                                    title={
                                      user?.isGuest
                                        ? "Disabled in demo mode — no real payments"
                                        : undefined
                                    }
                                  >
                                    <button
                              ...
                              {m.state === "SUBMITTED" && (
                                <>
                                  ...
                                  {m.submissionNote && (
                                    <span
                                      className="max-w-[160px] truncate text-xs italic text-gray-500"
                                      title={m.submissionNote}
                                    >
                                      "{m.submissionNote}"
                                    </span>
                                  )}
