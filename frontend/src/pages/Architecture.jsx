...
function Arrow({ vertical = false }) {
  return vertical ? (
    <div className="flex justify-center text-gray-400 text-lg leading-none py-1">
      ↓
    </div>
  ) : (
...
const TRANSITIONS = [
  ...
  },
  {
    from: "FUNDED",
    to: "IN_PROGRESS",
    trigger: 'Freelancer clicks "Start Work"',
  },
  {
    from: "IN_PROGRESS",
    to: "SUBMITTED",
    trigger: "Freelancer submits with note",
  },
  {
    from: "SUBMITTED",
    to: "APPROVED",
    trigger: "Client approves — payout triggered",
  },
  {
    from: "SUBMITTED",
    to: "DISPUTED",
    trigger: "Client or freelancer raises dispute",
  },
  {
    from: "DISPUTED",
    to: "APPROVED",
    trigger: "Admin resolves: approve (payout)",
  },
  {
    from: "DISPUTED",
    to: "REFUNDED",
    trigger: "Admin resolves: refund (Razorpay refund)",
  },
  {
    from: "CREATED",
    to: "CANCELLED",
    trigger: "Client cancels before payment",
  },
  {
    from: "FUNDED",
    to: "AUTO_REFUNDED",
    trigger: "Admin: freelancer unresponsive",
  },
];
...
const MODELS = [
  ...
  },
  {
    ...
    fields: [
      ...
      "★ milestones: [MilestoneSchema] (embedded)",
    ],
  ...
  },
  {
    ...
    fields: [
      ...
      "state: 9-value enum",
      "razorpayOrderId, razorpayPaymentId, razorpayPayoutId",
      "submissionNote, submittedAt, approvedAt, refundedAt",
    ],
  ...
  },
  {
    ...
    fields: [
      "userId → User",
      "action: String (e.g. MILESTONE_FUNDED)",
      "entityType, entityId",
      "metadata: Mixed, timestamp",
    ],
  ...
  },
  {
    ...
    color: "gray",
    fields: [
      "projectId → Project (compound unique index)",
      "fromUserId → User",
      "toUserId → User",
...
export default function Architecture() {
  useEffect(() => {
    document.title = "Architecture — WorkNest";
  }, []);
    ...
    <ShowcaseLayout>
      <div className="mx-auto max-w-6xl px-6 py-16">
        ...
        <p className="mb-12 max-w-2xl text-lg text-gray-600">
          Every diagram on this page is derived directly from the source code —
          no hand-wavy blocks.
        </p>
        ...
        <section className="mb-16">
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-100 bg-gray-50 p-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              <Box
                label="React 19 + Vite"
                sub="Zustand · react-router-dom v7"
                color="indigo"
              />
              <Arrow />
              <Box
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-center">
                ...
                <div className="text-xs text-amber-600">
                  Orders · Webhooks · Payouts (mocked)
                </div>
              </div>
        ...
        <section className="mb-16">
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-100 bg-gray-50 p-6">
            <div className="flex flex-wrap items-start justify-center gap-2">
              ...
              <Box
                label="jwt.sign()"
                sub="{ userId, role, isGuest }"
                color="blue"
              />
              <Arrow />
              <Box
                label="httpOnly cookie"
                sub="secure + sameSite:none in prod"
                color="amber"
              />
            </div>
            <div className="mt-4 flex flex-wrap items-start justify-center gap-2">
              <Box
                ...
                sub="calls GET /api/auth/me"
                color="violet"
              />
              <Arrow />
              <Box
                label="requireAuth middleware"
                sub="verifies cookie JWT"
                color="gray"
              />
              <Arrow />
              <Box
                label="Role-gated route"
                sub="client | freelancer | admin"
                color="emerald"
              />
            </div>
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              <strong>httpOnly means JS cannot read the cookie.</strong>{" "}
              Socket.io auth reads it server-side from{" "}
        ...
        <section className="mb-16">
          ...
          <p className="mb-4 text-sm text-gray-600">
            9 states · 9 transitions · all wrapped in MongoDB transactions ·
            AuditLog written on every transition.
          </p>
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-100 bg-gray-50 p-6">
            ...
            <p className="mt-6 mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500">
              Branch paths
            </p>
            <div className="flex flex-wrap gap-3">
              {[
                {
                  states: ["SUBMITTED", "DISPUTED", "APPROVED"],
                  label: "dispute → admin approve",
                },
                {
                  states: ["SUBMITTED", "DISPUTED", "REFUNDED"],
                  label: "dispute → admin refund",
                },
                { states: ["CREATED", "CANCELLED"], label: "client cancels" },
                {
                  states: ["FUNDED", "AUTO_REFUNDED"],
                  label: "admin auto-refund",
                },
              ].map(({ states, label }) => (
                <div
                  key={label}
                  className="flex-1 min-w-fit rounded-lg border border-gray-200 bg-white p-3"
                >
                      ...
                      <div key={s} className="flex items-center gap-1">
                        <span
                          className={`rounded px-2 py-0.5 text-xs font-medium ${
                            STATE_COLORS[s] === "emerald"
                              ? "bg-emerald-100 text-emerald-800"
                              : STATE_COLORS[s] === "amber"
                                ? "bg-amber-100 text-amber-800"
                                : STATE_COLORS[s] === "rose"
                                  ? "bg-rose-100 text-rose-800"
                                  : STATE_COLORS[s] === "violet"
        ...
        <section className="mb-16">
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-100 bg-gray-50 p-6">
            <div className="flex flex-wrap items-center justify-center gap-2">
              ...
              <Arrow />
              <Box
                label="/chat namespace"
                sub="cookie-authed via socketAuth"
                color="violet"
              />
              <Arrow />
              ...
              {[
                {
                  event: "join_project",
                  dir: "→ server",
                  desc: "Subscribe to project room (membership verified)",
                },
                {
                  event: "message:send",
                  dir: "→ server",
                  desc: "Persist to MongoDB + broadcast to room",
                },
                {
                  event: "message:typing",
                  dir: "→ server",
                  desc: "Relay typing indicator (not persisted)",
                },
                {
                  event: "message:new",
                  dir: "← client",
                  desc: "Populated message (senderId populated)",
                },
                {
                  event: "user:joined",
                  dir: "← client",
                  desc: "Presence signal on room join",
                },
                {
                  event: "user:left",
                  dir: "← client",
                  desc: "Presence signal on disconnect",
                },
              ].map((e) => (
                <div
                  key={e.event}
        ...
        <section className="mb-8">
          ...
          <p className="mb-4 text-sm text-gray-600">
            6 Mongoose models · milestones are{" "}
            <strong>embedded subdocuments</strong> inside Project (not a
            separate collection) — a deliberate design choice that keeps the
            escrow transaction atomic.
          </p>
