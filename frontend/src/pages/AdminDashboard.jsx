    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <p className="mb-4 text-sm text-gray-500">
          ...
          {dispute.milestone?.amount != null &&
            ` — ${formatINR(dispute.milestone.amount)}`}
          <br />
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50">
            ...
            <div>
              <p className="text-sm font-medium text-green-700">
                Resolve for Freelancer (Approve + Payout)
              </p>
              <p className="text-xs text-gray-500">
                Approve the work and release payment to freelancer's wallet.
              </p>
            </div>
          </label>
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Refund the milestone amount to the client.
              </p>
            </div>
          </label>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-6 py-4">
        <h1 className="text-xl font-bold text-indigo-700">WorkNest — Admin</h1>
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${M_STATE_STYLES.DISPUTED}`}
                                >
                                  DISPUTED
                                </span>
                              </div>
                              <p className="mt-1 text-sm text-gray-500">
                                ...
                                {d.milestone?.amount != null &&
                                  ` — ${formatINR(d.milestone.amount)}`}
                              </p>
                              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                                <span>
                                  Client: {d.project?.clientId?.name ?? "—"}
                                </span>
                                <span>
                                  Freelancer:{" "}
                                  {d.project?.freelancerId?.name ?? "—"}
                                </span>
                                <span>
                    ...
                    <div className="space-y-2">
                      {resolvedDisputes.map((d) => (
                        ...
                        >
                            ...
                            <span
                              className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${
                                d.resolution === "approve"
                                  ? M_STATE_STYLES.APPROVED
                                  : M_STATE_STYLES.REFUNDED
                              }`}
                            >
                              {d.resolution === "approve"
                                ? "APPROVED"
                          ...
                          <p className="mt-1 text-xs text-gray-400">
                            Resolved by {d.resolvedBy?.name ?? "admin"} ·{" "}
                            {d.resolvedAt ? timeAgo(d.resolvedAt) : "—"}
                          </p>
                        </div>
                      ))}
                    </div>
            ...
            {auditLoading ? (
              <div className="space-y-1">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className="animate-pulse flex items-center gap-3 rounded-lg border bg-white px-3 py-2"
                  >
              ...
              <div className="space-y-1">
                {auditLogs.map((log) => (
                  <div
                    key={log._id}
                    className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border bg-white px-3 py-2 text-xs"
                  >
